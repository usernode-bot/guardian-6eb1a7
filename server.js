...
// Import services
const guardianService = require('./src/services/guardianService.js');
const eligibilityService = require('./src/services/eligibilityService.js');
const fgService = require('./src/services/fgService.js');
const peerService = require('./src/services/peerService.js');
...
// Node status API (via peerService)
app.get('/api/node', async (req, res) => {
  try {
    const status = await peerService.getNodeStatus();
    res.json(status);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
// FG Service API (via fgService)
app.get('/api/fg', async (req, res) => {
  try {
    const fgData = await fgService.getFGData(req.user?.id || '-1');
    res.json(fgData);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
// Eligibility API (via eligibilityService)
app.get('/api/eligibility', async (req, res) => {
  try {
    const result = await eligibilityService.checkEligibility(req.user?.id || '-1');
    res.json(result);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
// Guardian Collection Gallery API (via guardianService)
app.get('/api/guardians', (req, res) => {
  try {
    ...
    res.json({
      ...
      allocated: guardians.filter(g => g.allocated).length,
      unallocated: guardians.filter(g => !g.allocated).length
    });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
// Guardian Allocation API (via guardianService + eligibilityService)
app.post('/api/guardians/allocate', async (req, res) => {
  try {
    if (!IS_STAGING) {
    ...
    const eligibility = await eligibilityService.checkEligibility(req.user?.id || '-1');
    if (!eligibility.eligible) {
      return res.status(400).json({ error: 'Not eligible', reasons: eligibility.reasons });
    }

    const preferredTier = req.body?.tier || null;
    const guardian = guardianService.allocateGuardian(req.user?.id || '-1', preferredTier);
    if (!guardian) {
